...
# from soda_model.logging import logging
logger = get_logger(__name__)


class MyTripletEvaluator(SentenceEvaluator):
    """
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        # Cosine distance
        pos_cos_distance = 1 - paired_cosine_distances(embeddings_anchors, embeddings_positives)
        neg_cos_distances = 1 - paired_cosine_distances(embeddings_anchors, embeddings_negatives)
        ...
        loss_cos = F.relu(torch.Tensor(pos_cos_distance) - torch.Tensor(neg_cos_distances) + self.triplet_margin).mean().numpy()
        loss_euclidean = F.relu(torch.Tensor(pos_euclidean_distance) - torch.Tensor(neg_euclidean_distances) + self.triplet_margin).mean().numpy()
        loss_manhattan = F.relu(torch.Tensor(pos_manhattan_distance) - torch.Tensor(neg_manhattan_distances) + self.triplet_margin).mean().numpy()
        ...
        logger.info("Loss Cosine Distance:   \t{:.2f}".format(loss_cos))
        logger.info("Loss Manhattan Distance:\t{:.2f}".format(loss_euclidean))
        logger.info("Loss Euclidean Distance:\t{:.2f}\n".format(loss_manhattan))
        ...
        if self.main_distance_function == SimilarityFunction.COSINE:
            return float(loss_cos)
        if self.main_distance_function == SimilarityFunction.MANHATTAN:
            return float(loss_manhattan)
        if self.main_distance_function == SimilarityFunction.EUCLIDEAN:
            return float(loss_euclidean)

        return max(loss_cos, loss_manhattan, loss_euclidean)
...
class MySentenceTransformer(nn.Sequential):
    ...
    def fit(self,
            ...
            log_steps: int = 0,
            log_callable: Callable[[int, int, float, float], None] = None,
            use_amp: bool = False,
        ...
        :param log_callable: Callback function that is invoked to log during training:
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, callback):
        ...
        # Here I should also shou the triplet loss. The point is how
        ...
        if evaluator is not None:
            ...
            self.log_callable(self.train_idx, self.training_steps, self.global_step, self.scheduler.get_last_lr(), score, mode="eval")
    ...
    @staticmethod
    def load(input_path):
        return MySentenceTransformer(input_path)
