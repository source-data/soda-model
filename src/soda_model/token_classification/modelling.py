...
class BertForMultiRolesClassification(BertPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        is_category: Optional[torch.Tensor] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], TokenClassifierOutput]:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
        ...
        concat_sequence = torch.concat((sequence_output, is_category[:, :, None]), dim=-1,)  # type: ignore

        sequence_output = self.dropout(concat_sequence)
        logits = self.classifier(sequence_output)
